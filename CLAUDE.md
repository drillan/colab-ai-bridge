...
このプロジェクトでは`uv`をパッケージマネージャーとして使用しています。

```bash
# 依存関係のインストール（プロジェクトの初期セットアップ）
uv sync

# 開発用依存関係を含むすべての依存関係のインストール
uv sync --all-groups
...
このプロジェクトでは`pytest`をテストフレームワークとして使用しています。
...
# カバレッジレポート付きで実行
uv run pytest --cov=src/colab_pydantic_ai --cov-report=html
```
...
このプロジェクトでは厳格な型安全性を要求しています。

要件:
- すべての関数、メソッド、変数に型アノテーションを付与
- 静的型チェック（mypy）をコミット前に実行
